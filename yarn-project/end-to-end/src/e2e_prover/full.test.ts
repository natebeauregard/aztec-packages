...
describe('full_prover', () => {
  ...
  it(
    ...
    async () => {
      ...
      // TODO: add to ddos test
      // override clientIvcVkBuffer with a bit flip (override the first byte of the proof to make invalid proving time fast)
      // privateProvenTx.clientIvcProof.clientIvcVkBuffer = overriden first bit
      ...
      logger.info(`Found proof for the previous epoch`);
  ...
  // it('should prevent txs with invalid proofs from causing ddos attacks', async () => {
  //   if (!REAL_PROOFS) {
  //     t.logger.warn(`Skipping test with fake proofs`);
  //     return;
  //   }
  ...
  //   const interaction = provenAssets[0].methods.transfer(recipient, 1n);
  ...
  //   logger.info(`Only valid tx has been mined`);
  ...
  //   await validTx.wait({ timeout: 300, interval: 10, proven: true, provenTimeout: 1500, debug: true });

  //   logger.info(`Valid tx was proven`);

  //   // Assert that the large influx of invalid txs does not ddos the node and that the valid tx is mined
  //   // for (let i = 0; i < NUM_INVALID_TXS; i++) {
